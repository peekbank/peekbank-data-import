...
stimuli_data <- target_distractors %>% # 48 entries
  ...
  ) %>% # 32 distinct labels with image paths lab ids
  mutate(
...
mega_trials_table <- target_distractors %>%
  ...
  ) %>% # extract target side and distractor label based on word column
  ...
  ) %>% # finish adding target and distractor stimulus ids
  select(-left, -right) %>%
...
trial_types_data <- mega_trials_table %>%
  ...
  select(
    trial_type_id, full_phrase, full_phrase_language, point_of_disambiguation, target_side,
...
timepoint_data <- raw_timepoint_data %>%
  mutate(
    ...
    lab_trial_id = case_when(
      left_pic %in% unique(trial_types_data$lab_trial_id) ~ left_pic, # phew, this is necessary because sometimes the distractor is the target, and vice versa
