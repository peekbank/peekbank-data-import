...
plot_timecourse <- function(df, filter_crit, facet_type) {
  ...
  df %>%
    ...
    guides(color = F) +
    labs(
      x = "Time in msec from onset of noun",
...
make_stimulus_key <- function(xml_obj, order_name, order_age) {
  ...
  d %>% mutate(
    stimulus_name = str_remove(stimulus_name, ".mov|.jpg|.png|.avi"),
...
process_one_stim_xml <- function(xml_obj, x_max = 1920, y_max = 1080, file_name) {
  ...
  tibble(
    ...
    mutate(
      aoi_type = ifelse(str_detect(stimulus_name, ".jpg|.png"), "image", "movie"),
...
fit_ddm <- function(df, condition_col, subid_col, bysub = T, niter = 500) {
  ...
  if (nrow(df) > 0) {
    ...
    fit.vals <- optim(c(1, .1, .1, 1), wiener_deviance,
...
remove_extreme_vals <- function(data_frame, sd_cut_val = 2, value_column) {
  ...
  data_frame %<>%
    mutate(
      cut_point_upper = cut_point_upper,
      cut_point_lower = cut_point_lower
    ) %>%
    filter_(
      filter_criteria_upper,
...
preprocess.data <- function(d, x.max = 1920, y.max = 1080,
  ...
  if (avg.eyes) {
    ...
    d %<>%
      mutate(
        x = ifelse(!is.na(rx) & !is.na(lx), (lx + rx) / 2,
