...
process_subjects_info <- function(file_path) {
  data <- read.csv(file_path) %>%
    ...
    dplyr::mutate(
      age = lab_age * 12, # convert age to months
      lab_age_units = "years"
    ) %>%
    mutate(
      sex = factor(sex, labels = c("male", "female", "unspecified")), # this is pulled from yurovsky processing code
...
process_smi_trial_info <- function(file_path) {
  ...
  trial_types_data <- trial_types_data %>%
    mutate(
      Stimulus = str_remove_all(Stimulus, ".jpg"),
      stimulus_name = str_remove_all(Stimulus, "o_|t_")
    ) %>%
    rename("target_side" = "target") %>%
    separate(stimulus_name,
      into = c("left_image", "right_image"), sep = "_",
      remove = F
    ) %>%
    mutate(
