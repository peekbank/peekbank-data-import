...
administrations_table <- raw_subjects_table %>%
  select(subject_id, AgeInMonths, lab_subject_id) %>%
  mutate(
    dataset_id = dataset_id,
    monitor_size_x = 1366,
    monitor_size_y = 768,
...
aoi_region_sets <- tibble(
  "aoi_region_set_id" = 0, # only one set of aoi regions for this study
  "l_x_max" = 395,
  "l_x_min" = 359,
  "l_y_max" = 754,
  "l_y_min" = 359,
...
trials_types_table$distractor_word <- as.character(lapply(
  trials_types_table$distractor_image,
  function(word_png) {
    return(str_replace(word_png, ".png", ""))
  }
))

trials_types_table$trial_type_id <- as.numeric(lapply(
  trials_types_table$lab_trial_id,
  function(lab_trial) {
    return(str_replace(
      lab_trial,
      "LWL", ""
...
trials_types_table <- trials_types_table %>%
  ...
  mutate(vanilla_trial = TRUE, trial_type_aux_data = NA)
...
get_administration_aoi <- function(administration) {
  ...
  participant_info <- merge(
    administrations_table %>% select(administration_id, subject_id),
  ...
  lwl_file <- readr::read_delim(fs::path(read_path, "TXT_exported", participant_info$file[1]),
    delim = "\t",
  ...
  administration_aoi_data <- administration_aoi_data %>% mutate(fixed_aoi = case_when(
    aoi == "L" ~ "right",
    aoi == "R" ~ "left",
    TRUE ~ aoi
  ))
  ...
  administration_aoi_data <- administration_aoi_data %>% select(
  ...
  administration_aoi_data <- merge(
    administration_aoi_data,
    trials_types_table %>%
...
d_tidy <- aoi_timepoints_table %>%
  left_join(trials_table, by = "trial_id") %>%
  left_join(administrations_table) %>%
