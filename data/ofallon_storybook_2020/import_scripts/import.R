...
read_path <- here("data/ofallon_storybook_2020/raw_data")
write_path <- here("data/ofallon_storybook_2020/processed_data")
...
organize_trial_data <- function(trial_filepath){
  ...
  trial_data <- trial_data[trial_data$word != "WORD",] %>% 
    mutate(correct_side_opposite = case_when(correct_side_opposite == 'LEFT' ~ 'L',
...
unique_trials_data <- unique_trials_data %>% 
  separate(col = trial_group, 
           into = c("lab_subject_id",
                    "trial_group"),
           sep = "_", remove = T) %>% 
  mutate(trial_group = paste("ORDER", 
...
unique_trials_tidy <- unique_trials_data %>% 
  ...
  ungroup() %>%
  rename(full_phrase = word, 
         target_side = correct_side_opposite) %>%
  mutate(target_side = case_when(target_side == "L" ~ "left", 
                            target_side == "R" ~ "right", 
                            TRUE ~ "ambig"))
