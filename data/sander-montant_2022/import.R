...
only_relevant <- function(df) {
  df |>
    filter(str_detect(RecordingName, "Mix[-_]14") == TRUE) |>
    mutate(
...
exclusions <- demog_data |>
  ...
  mutate(
    exclusion_reason = sapply(exclusions, \(e) {
      reason_str <- e |>
        filter(excluded) |>
...
cdi_data <- eval(sym(cdi_files)) |>
  ...
  summarise(
    rawscore = sum(produces_word),
    .groups = "drop"
  ) |>
  mutate(
    study_id = study_id |> str_remove(", .*"),
    measure = "prod",
    form = tolower(form),
    percentile = NA
  ) |>
