...
Then, set up general parameters and file paths.

```{r parameters, message = FALSE, warning = FALSE}
dataset_name <- "reflook_socword"
source(here("data/reflook_socword/import_scripts/import_helpers_template.R"))
dataset_id <- 0
file_ext = '.txt'
# raw data directory
dir_path <- fs::path(here::here("data", dataset_name, "raw_data"))
## paths -- specific for this dataset ##
...
##create stimuli data
stimuli.data <- process_smi_stimuli(paste0(exp_info_path, "/reflook_v3_tests.csv")) %>%
...
That's it! Finally, we'll write all the tables to `.csv` files and validate them to make sure they adhere to the schema. These steps are commented out so you don't accidentally write a bunch of stuff when running this script. There you have it--that's how to import a Peekbank eyetracker dataset.
