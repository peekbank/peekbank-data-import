...
digest.dataset <- function(
  ...
  stimuli <- data %>% 
    select(target_stimulus_label_original,
           target_stimulus_label_english,
           target_stimulus_novelty,
           target_stimulus_image_path,
  ...
  data <- data %>%
    ...
    ungroup() %>%
    group_by(administration_id, trial_type_id, trial_index) %>%
    mutate(trial_id = cur_group_id() - 1) %>%
    ungroup() %>% 
    group_by(administration_id) %>%
    mutate(trial_order = cumsum(trial_type_id != lag(trial_id, default = first(trial_id)))) %>%
    ungroup()
  ...
  administrations <- data %>% 
    ...
    mutate(
      age = case_when(
        ...
        lab_age_units == "year" ~ 12*lab_age, # TODO decimal rule
        .default ~ NA
      ),
